...
token ="5559194720:AAEgixceQWufCQ2o0oxWIkHW6CeHZokFJ_A"
...
def Welcome(message):
    ...
    bot.reply_to(message,"Welcome to the TikTok account information bot",reply_markup=telebot.types.InlineKeyboardMarkup([
                     [telebot.types.InlineKeyboardButton(text='Dev',url='tg://user?id=1315011160')]]))
...
def info(message):
    ...
    if data:
        ...
        keyboard.add(
        telebot.types.InlineKeyboardButton(f'- name ({name})•', callback_data='1'),
        telebot.types.InlineKeyboardButton(f'- followers ({followers})•', callback_data='2'),
        telebot.types.InlineKeyboardButton(f'- following ({following})•',callback_data='3'),
        telebot.types.InlineKeyboardButton(f'- Last name change ({last})•',callback_data='4'),
        telebot.types.InlineKeyboardButton(f'- id ({id})•',callback_data="5"),
        telebot.types.InlineKeyboardButton(f'- date ({time})•',callback_data="6"),
        telebot.types.InlineKeyboardButton(f'- country ({country_emoji} - {acc})•',callback_data="7")
        )
